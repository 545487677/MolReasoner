...
# Note: make sure cp nltk_data to /root/ directory
cp -r verl/nltk_data /root/
# Note:  data.train_files and data.val_files should be set to the correct paths for your dataset.
# Note: actor_rollout_ref.model.path should point to the pre-trained model checkpoint you want
# to use for training.
# Note: trainer.project_name and trainer.experiment_name should be set to your desired project
  ...
  for lr in "${LRS[@]}"; do
    for r_n in "${ROLLOUT_NS[@]}"; do
      for temp in "${TEMPERATURES[@]}"; do
        ...
        exp_name="molecule_captioning_${TRAIN_BATCH_SIZES}_lr${lr}_rollout${r_n}_temp${temp}_lenght${LENGTH_TOKEN}"
        log_path="xxxxxx/xxx/_${exp_name}.log"
        save_dir="xxxx/molecule_captioning/${exp_name}"
        ...
        python3 -m verl.trainer.main_ppo \
          ...
          actor_rollout_ref.model.path=./MolReasoner/ckpt/sft/sft_molecule_captioning \
          ...
          trainer.project_name='molecule_captioning' \
          trainer.experiment_name=$exp_name \
          reward_model.exp_method=mol2desc_default \
